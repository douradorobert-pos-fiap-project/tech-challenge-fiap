...
class OrdemServicoModel(Base):
    ...
    cliente_id: Mapped[str] = mapped_column(
        String(36), ForeignKey("clientes.id"), nullable=False, index=True
    )
    veiculo_id: Mapped[str] = mapped_column(
        String(36), ForeignKey("veiculos.id"), nullable=False, index=True
    )
...
class ItemServicoModel(Base):
    ...
    ordem_servico_id: Mapped[str] = mapped_column(
        String(36), ForeignKey("ordens_servico.id"), nullable=False, index=True
    )
